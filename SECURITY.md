...
The supported security scope for `parse_example_swift` is the current default branch, `master`. Older commits, tags, branches, forks, demos, and generated artifacts are not actively supported unless the repository explicitly marks them as maintained.

Project summary: Swift Parse Example

## Reporting a Vulnerability

Please report suspected vulnerabilities through GitHub's private vulnerability reporting or by opening a draft GitHub Security Advisory for `garethpaul/parse_example_swift` when that option is available. If GitHub does not show a private reporting option for this repository, contact the repository owner through GitHub and avoid posting exploit details publicly until the issue can be assessed.
...
- Keep non-placeholder XCTest coverage, including the non-empty bundle
  ...
  backend data flows.
- Keep plist package types explicit so app and XCTest target metadata changes
  remain reviewable when Xcode is unavailable.
